...
final class Library: NSObject, ObservableObject, NSSoundDelegate {
    ...
    /// First launch: copy the bundled starter sounds into the user's library.
    /// Only runs when the library is completely empty, so it never overwrites
    /// or resurrects anything the user changed or deleted.
    private func seedStarterLibrary() {
        let fm = FileManager.default
        guard let bundled = Bundle.main.url(forResource: "Sounds", withExtension: nil),
              let existing = try? fm.contentsOfDirectory(atPath: dir.path),
              existing.filter({ $0 != ".DS_Store" }).isEmpty
        else { return }
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        // Touch the library up front so a fresh install seeds the starter
        // sounds at launch, not lazily on first use.
